...
if (Test-Path "aifitness.db") {
    ...
    Write-Host "- spring.jpa.hibernate.ddl-auto=validate (Hibernate validates schema only)"
    Write-Host "- spring.flyway.enabled=true (Flyway owns migrations)"
    Write-Host ""
    Write-Host "When you restart the backend, Flyway will apply any pending migrations"
    Write-Host "to ensure all required fields exist, including:"
